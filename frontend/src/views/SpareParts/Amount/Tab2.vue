 ...
 * @LastEditors: huangyh
 * @LastEditTime: 2022-03-23 11:07:28
-->
<template>
  <div ref="mainPanel" class="mainPanel">
    <div ref="searchPanel" class="searchPanel">
      ...
      <el-form ref="bynForm" :inline="true" size="small">
        <el-form-item>
          <el-button type="primary" size="mini" @click="apply">出库</el-button>
        </el-form-item>
      </el-form>
    </div>
      ...
      <el-table :data="dataList" style="width: 100%" height="100%" class="tableGrow">
        <el-table-column label="操作" min-width="80" fixed>
          <template slot-scope="scope">
            <el-button type="text" v-if="scope.row.state == 1" @click="cancelApply(scope.row)">取消出库</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="备件出库" width="70%" :visible.sync="dialogFormVisible" :modal-append-to-body="false">
      <el-table :data="applyTabList" height="200" class="tableGrow">
        ...
        <el-table-column prop="apply" label="出库数量" min-width="140">
          <template slot-scope="scope">
            <el-input
              type="number"
              style="width:100%;"
    ...
    <el-dialog title="查看" width="60%" :visible.sync="dialogFormCheck" :modal-append-to-body="false">
      <el-table :data="currentRow.parts" height="300px" ref="mStock" class="tableGrow">
        <el-table-column prop="name" label="备件名称" show-overflow-tooltip min-width="140" />
        <el-table-column prop="apply" label="出库数量" min-width="160" />
        <el-table-column prop="classify" show-overflow-tooltip label="备件类别" min-width="140">
          <template slot-scope="scope">{{ classify[scope.row.classify] }}</template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  computed: {
    ...mapGetters(["classify", "allStockList", "allStockOutList"]),
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      let searchData = this.allStockOutList.filter((i) =>
    ...
    confirmApply() {
      ...
      let temp = {
        ...
        type: 3// 1个人，2总仓库入库，3总仓库出库
      }
      this.$store.commit('demo/ADD_ALLSTOCKOUTLIST', temp);
      this.search();
    ...
    cancelApply(row) {
      this.$confirm("是否确认取消出库?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          this.$store.commit('demo/DEL_ALLSTOCKOUTLIST',row);
          this.search();
        })
        .catch(() => {
          this.$message({
